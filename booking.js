...
function getBookingPayload() {
  ...
  const urgent = ["urgent", "asap", "emergency", "immediately", "angry", "frustrated"].some((word) =>
...
async function onReschedule() {
  ...
  try {
    ...
    store.rescheduleInteraction(bookingState.bookingId, payload);
...
async function applyMissedCallContext() {
  ...
  try {
    const { missedCall } = await callbackApi.getMissedCall(requestToken);
